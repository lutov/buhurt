...
@section('content')
    ...
    $has_genres = (count($fav_gens_books) || count($fav_gens_films) || count($fav_gens_games) || count(
            $fav_gens_albums
        )) ? true : false;
    ...
    <section class="text-center">
        ...
        <ul class="list-inline">
            ...
            @if($has_genres)
                <li class="list-inline-item"><a href="#genres">Любимые жанры</a></li>@endif

            @if($has_rates)
                <li class="list-inline-item"><a href="#rates">Распределение оценок</a></li>@endif

            <li class="list-inline-item"><a href="#achievements">Достижения</a></li>

            @if($has_options)
                <li class="list-inline-item"><a href="#options">Настройки</a></li>@endif
    ...
    <div class="row">
        ...
        <div class="col-md-3 mb-4">
            ...
            <div class="card">
                ...
                <img src="/data/img/avatars/{!! $avatar !!}.jpg?hash={!! $hash !!}" alt="{!! $user->username !!}"
                     class="card-img-top"/>
        ...
        <div class="col-md-9 border rounded p-3">
            ...
            @if(0 != $books_wanted || 0 != $films_wanted || 0 != $games_wanted || 0 != $albums_wanted)
                <p>
                    ...
                    @if(0 != $books_wanted)
                        прочесть <a
                                href="{!! URL::action('User\UserController@wanted', array($user->id, 'books')) !!}">{!! TextHelper::number($books_wanted, array('книгу', 'книги', 'книг')) !!}</a>
                        ,
                    @endif
                    @if(0 != $films_wanted)
                        посмотреть <a
                                href="{!! URL::action('User\UserController@wanted', array($user->id, 'films')) !!}">{!! TextHelper::number($films_wanted, array('фильм', 'фильма', 'фильмов')) !!}</a>
                        ,
                    @endif
                    @if(0 != $games_wanted)
                        сыграть в <a
                                href="{!! URL::action('User\UserController@wanted', array($user->id, 'games')) !!}">{!! TextHelper::number($games_wanted, array('игру', 'игры', 'игр')) !!}</a>
                        ,
                    @endif
                    @if(0 != $albums_wanted)
                        слушать <a
                                href="{!! URL::action('User\UserController@wanted', array($user->id, 'albums')) !!}">{!! TextHelper::number($albums_wanted, array('альбом', 'альбома', 'альбомов')) !!}</a>
                    @endif
                </p>
            @endif
            ...
            @if(0 != $books_unwanted || 0 != $films_unwanted || 0 != $games_unwanted || 0 != $albums_unwanted)
                <p>
                    ...
                    @if(0 != $books_unwanted)
                        читать <a
                                href="{!! URL::action('User\UserController@unwanted', array($user->id, 'books')) !!}">{!! TextHelper::number($books_unwanted, array('книгу', 'книги', 'книг')) !!}</a>
                        ,
                    @endif
                    @if(0 != $films_unwanted)
                        смотреть <a
                                href="{!! URL::action('User\UserController@unwanted', array($user->id, 'films')) !!}">{!! TextHelper::number($films_unwanted, array('фильм', 'фильма', 'фильмов')) !!}</a>
                        ,
                    @endif
                    @if(0 != $games_unwanted)
                        играть в <a
                                href="{!! URL::action('User\UserController@unwanted', array($user->id, 'games')) !!}">{!! TextHelper::number($games_unwanted, array('игру', 'игры', 'игр')) !!}</a>
                        ,
                    @endif
                    @if(0 != $albums_unwanted)
                        слушать <a
                                href="{!! URL::action('User\UserController@unwanted', array($user->id, 'albums')) !!}">{!! TextHelper::number($albums_unwanted, array('альбом', 'альбома', 'альбомов')) !!}</a>
                    @endif
                </p>
            @endif
    ...
    @if($has_genres)
        ...
        <div class="row">
            ...
            <div class="col-md-12 border rounded p-3">
                ...
                @if(count($fav_gens_books))<p>
                    Книги: {!! DatatypeHelper::arrayToString($fav_gens_books, ', ', '/genres/'); !!}</p>@endif
                @if(count($fav_gens_films))<p>
                    Фильмы: {!! DatatypeHelper::arrayToString($fav_gens_films, ', ', '/genres/'); !!}</p>@endif
                @if(count($fav_gens_games))<p>
                    Игры: {!! DatatypeHelper::arrayToString($fav_gens_games, ', ', '/genres/'); !!}</p>@endif
                @if(count($fav_gens_albums))<p>
                    Альбомы: {!! DatatypeHelper::arrayToString($fav_gens_albums, ', ', '/genres/'); !!}</p>@endif
    ...
    @if($has_options)
        ...
        <div class="row mt-5 align-top">
            ...
            <div class="col-md-12 align-top border rounded p-3">
                ...
                <p>
                    <a href="{!! URL::action('User\UserController@change_password') !!}" type="button"
                       class="btn btn-secondary">
                        Сменить пароль
                    </a>
                </p>
