        ...
        <div class="row mt-5">
            <div class="col-md-9">
                <div class="card @include('widgets.card-class')">
                    {!! Form::open(array('action' => 'Admin\DatabaseController@save', 'class' => 'add_person', 'method' => 'POST', 'files' => true)) !!}
                    <div class="card-header">
                        <h1 class="card-title">@yield('title')</h1>
                        <h2 class="card-subtitle mb-2 text-muted">@yield('subtitle')</h2>
                        {!! Form::hidden('action', 'edit') !!}
                        {!! Form::hidden('section', 'persons') !!}
                        {!! Form::hidden('element_id', $element->id) !!}
                    </div>
                    <div class="card-body">
                        <p>{!! Form::text('name', $element->name, array('placeholder' => 'Имя', 'id' => 'game_name', 'class' => 'form-control w-100')) !!}</p>
            ...
            <div class="col-md-3">
                <div class="card @include('widgets.card-class')">
                    ...
                    <div id="transfer" class="card-footer text-center">
                        @include('widgets.card-transfer', array('controller' => 'PersonsController', 'element' => $element))
                    </div>
                </div>
            </div>
        </div>
